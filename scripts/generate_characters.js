...
let characters = []
  ...
  .map((rawCard) => {
    ...
    card.damageTypes = [];
    if (rawCard.sides) {
      card.damageTypes = _.uniq(
        rawCard.sides.reduce((acc, val) => {
          if (val.includes('ID')) {
            acc.push('ID');
          }
    ...
    if (card.id === '05050') {
      // CHARACTER: Zeb Orrelios, 05050
      card.damageTypes.push('RD');
    }

    if (card.id === '07056') {
      // CHARACTER: Luke Skywalker, 07056
      card.damageTypes.push('MD');
    }

    if (card.id === '09018') {
      // CHARACTER: Captain Phasma, 09018
      card.damageTypes.push('MD');
    }

    if (card.id === '09075') {
      // CHARACTER: Padmé Amidala, 09075
...
characters = characters.map((card, index) => ({
...
console.log(`Output ${characters.length} characters...`);
jsonfile.writeFile(
  path.join(__dirname, '../data/characters.json'),
  charactersObject,
);
jsonfile.writeFile(
  path.join(__dirname, '../data/characters_stats.json'),
  charactersStats,
);
