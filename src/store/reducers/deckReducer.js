...
const deckReducer = (state = initialState, action) => {
  switch (action.type) {
    case 'ADD_CHARACTER_TO_DECK': {
      const existingCardIndex = state.characters.findIndex(
        (characterObject) => characterObject.id === action.payload.characterId,
      );
      ...
      return {
        ...
        affiliation:
          state.affiliation === 'neutral'
            ? action.payload.characterAffiliation
            : state.affiliation,
        characters: [...state.characters, characterObject],
      };
    }
    ...
    case 'REMOVE_CHARACTER_FROM_DECK': {
      ...
      const characters = state.characters.reduce(
        (array, characterObject, index) => {
          if (index === existingCardIndex) {
    ...
    case 'INCLUDE_CHARACTER': {
      const existingIndex = state.excludedCharacterIds.findIndex(
        (excludedCharacterId) =>
          excludedCharacterId === action.payload.characterId,
      );

      if (existingIndex !== -1) {
        return {
          ...state,
          excludedCharacterIds: _without(
